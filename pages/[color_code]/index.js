...
const Color = () => {
  ...
  const handleSubmit = event => {
    ...
    color_redirect = color_redirect[0] === '#' ? color_redirect.substring(1) : color_redirect;
    const reg = /^([0-9A-F]{3}){1,2}$/i;

    console.log(color_redirect);
    console.log(colorNameToHex(color_redirect));

    if(reg.test(color_redirect)) {
        color_redirect = color_redirect.toUpperCase();
        console.log(color_redirect)

        router.push('/' + color_redirect);
    } else if (colorNameToHex(color_redirect)) {
        color_redirect = colorNameToHex(color_redirect).toUpperCase().substring(1);
        router.push('/' + color_redirect);
    }
    else { router.push('/') }
